...
questions = copy.deepcopy(quiz_questions) # made deep copy of the original questions dictionary 
# so that any changes made to the copy won't affect the original, i.e. 'shuffle'
...
#    questions_shuffled = shuffle(questions) 
    ...
    # which is more like a set than a list, so it couldnt be indexed. 
    # Therefore the solution to the problem was to pass list(d.items()) to random
    ...
    # the values are returned to template/main.html

@app.route('/quiz', methods=['POST'])
def quiz_answers():
    answered = request.form
    # return list(answered.keys())[0]
    if questions[list(answered.keys())[0]][0] == list(request.form.values())[0]:
        return "Correct"
    else:
        return "Incorrect"
